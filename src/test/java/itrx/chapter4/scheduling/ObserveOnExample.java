...
public class ObserveOnExample {
    ...
    public void testObserveOnBeforeAfter() {
        ...
        Assert.assertEquals("Create on main thread", threads[0], threads[1]);
        Assert.assertEquals("Synchronous before observeOn", threads[1], threads[2]);
        Assert.assertEquals("Synchronous after observeOn", threads[3], threads[4]);
